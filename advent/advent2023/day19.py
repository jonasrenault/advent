...
def apply_rules(
    rules: dict[str, list[tuple[str, Callable[[int, int], bool], int, str] | tuple[str]]],
    part: dict[str, int],
...
def parse_rule(line: str) -> tuple[str, list[Any]]:
    ...
    for rule in rules:
        if ":" not in rule:
            ...
            conditions.append((rating, op, val, dest))  # type: ignore
