...
def grid_to_graph(
    ...
    """
    Transform grid to a graph. If ignore_slopes is True, slopes are ignored and graph is
    simplified by merging nodes that have only two neighbors, keeping only intersections
    as nodes.
    ...
    Returns:
        dict[tuple[int, int], set[tuple[tuple[int, int], int]]]: the graph
    """
...
def adjacent_nodes(
    ...
    """
    Yield the adjacent nodes and their distance from the curr position. Nodes are grid
    positions that are either the start or end, or that have more than 2 neighbors.
...
def is_node(
    ...
    """
    Check if node position is a node in the graph, i.e. is the start or end pos, or has
    ...
    """
    return (
        node == src or node == end or len(list(neighbors(grid, node, ignore_slopes))) > 2
    )
...
def longest_path(
    ...
    """
    Find the longest path in the graph, from curr position to end, given dist and
    traversed nodes to reach curr position.

    Args:
        graph (dict[tuple[int, int], set[tuple[tuple[int, int], int]]]): the graph
        end (tuple[int, int]): the end position
