...
def search(row: str, idx: int, broken: tuple[int, ...], chunk_length: int) -> int:
    ...
    if row[idx] in ".?":
        ...
        # check current chunk has correct length and move to next char,
        # resetting current chunk to 0
...
def arrangements(row: str, broken: tuple[int, ...]) -> list[str]:
    """
    Return all possible combinations of replacing ? in row by a #. Not all arrangements
    are valid.
