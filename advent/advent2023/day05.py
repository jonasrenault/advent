...
def read_puzzle(lines: list[str]) -> list[int]:
    # Read seed numbers
    seeds = lines[0][lines[0].index(":") + 1 :].split(" ")
    seeds_nb = [int(s.strip()) for s in seeds if s]
...
def convert_range(src: int, rng: int, key: str) -> list[tuple[int, int]]:
    """
    Convert a src range using key's map. Return a list of (start, range) tuples.
...
def intersect(
    ...
    """
    Find the intersection of two (start, range) intervals. Returns the intersection
    interval, as well as the remaining intervals outside of the intersection.
