...
def get_loop(grid: npt.NDArray[np.str_]) -> list[tuple[int, int]]:
    """
    Starting from S position, follow the pipes until we loop back to S. Since we don't
    know what S is, try out all of its neighbors until we loop back to S, or until an
    exception is raised in which case no loop was found.
    ...
    S: tuple[int, int] = start[0][0], start[1][0]
...
def intersections(grid: npt.NDArray[np.str_], x: int, y: int) -> int:
    """
    Count the # of intersections by drawing a line to the right of the point.
    Intersections are |, or any other section of pipe which is not horizontal (i.e L---J
    or F---7 are horizontal lines, whereas L---7 or F---J are not).
