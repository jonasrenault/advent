...
def main():
    ...
    # Build a dict of the indices for all the * chars, with a set of their
    # adjacent numbers
    ...
    # Sum the product of adjacent numbers for all the gears that have exactly
    # two adjacent numbers
    advent.submit(2, sum([prod(g) for g in gears.values() if len(g) == 2]))


def get_number_indices(puzzle: list[str]) -> list[tuple[int, int, int]]:
    """
...
def is_part(puzzle: list[str], index: tuple[int, int, int]) -> bool:
    """
...
def get_number(puzzle: list[str], index: tuple[int, int, int]) -> int:
    """
    Get number value.
    """
...
def get_gears(puzzle: list[str], index: tuple[int, int, int]) -> list[tuple[int, int]]:
    """
