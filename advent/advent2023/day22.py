...
class Brick:
    ...
    def overlaps(self, other: Brick) -> bool:
        for l, r in product(self.cubes, other.cubes):  # noqa: E741
...
def get_supports(
    ...
    Returns:
        tuple[dict[str, set[str]], dict[str, set[str]]]: dicts of supported_by
        and supports
    """
    ...
    for brick_id, on_top in supported_by.items():
        for ob_id in on_top:
            supports[ob_id].add(brick_id)

    return supported_by, supports


def get_removable(bricks: list[Brick], supported_by: dict[str, set[str]]) -> set[str]:
    """
    ...
    Returns:
        set[str]: set of bricks which can be removed
    """
...
def get_bricks(lines: list[str]) -> list[Brick]:
    ...
    for line in lines:
        ll, lr = line.split("~")
        left = tuple([int(x) for x in ll.split(",")])
        right = tuple([int(x) for x in lr.split(",")])
        bricks.append(Brick(left, right))  # type: ignore
    return bricks
