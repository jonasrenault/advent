...
class Module(ABC):
    @abstractmethod
    def process(self, pulse: str, sender: str) -> None | list[tuple[str, str]]:
        pass


class FlipFlop(Module):
    def __init__(self, children: Iterable[str]) -> None:
...
class Conjunction(Module):
    ...
    def __str__(self) -> str:
        return (
            "&" + " > " + ", ".join(self.memory.keys()) + "&->" + ", ".join(self.children)
        )
...
def main():
    ...
    for _ in tqdm(range(1000)):
        l, h, _ = press_btn(modules)  # noqa: E741
        low += l
        high += h
    ...
    for input in modules["dn"].memory.keys():
        pulse = "high"
...
def press_btn(
    ...
    """
    Press button module, sending a low pulse to broadcaster module and all processing all
    subsequent pulses. Returns the count of low and high pulses sent. Function will return
    True as third tuple element if stop_pulse is provided and was sent as a result of
    pushing the button.
